    ...
    //TODO:  Validate all members of QuadTree
    {
        ...
        public void Add(T item)
        {
            ...
            if (Root == null)
            {
                ...
                Root = new Node(itemBoundary);
                Extent = itemBoundary;
                Root.Contents.Add(item, itemBoundary);
                Count = 1;
                return;
            }
            ...
            if (!Root.Boundary.Contains(itemBoundary))
            {
        ...
        {
            //Step #1 - find the contianing node.  
            Rect itemBoundary;
            ...
            if (itemBoundary.Top == Root.Boundary.Top || itemBoundary.Bottom == Root.Boundary.Bottom || itemBoundary.Left == Root.Boundary.Left || itemBoundary.Right == Root.Boundary.Right)
                UpdateExtent();
        ...
        {
            if (Root == null) return;
            foreach (T item in GetDescendantNodes(Root).SelectMany((node) => node.Contents.Keys))
            {
        ...
        private Node FindContainingNode(T item)
        {
            ...
            if (Root == null || !Root.Boundary.Contains(boundary)) return null;
            Node focus = Root;
            while (true)
            {
        ...
        public IEnumerator<T> GetEnumerator()
        {
            ...
            List<Node> allNodes = GetDescendantNodes(Root);
        ...
        /// <summary>
        /// Returns the set of all items contained on this tree that intersect the given rect.
        /// </summary>        
        public ISet<T> GetIntersection(Rect other)
        {
            HashSet<T> result = new HashSet<T>();
            if (Root == null) return result;
            ...
            {
                Node focus = stack.Pop();
                if (!focus.Boundary.IntersectsWith(other)) continue;
                foreach (T item in focus.Contents.Keys)
                    if (focus.Contents[item].IntersectsWith(other)) result.Add(item);
                if (focus.Children == null) continue;
                stack.Push(focus.Children[0]);
                stack.Push(focus.Children[1]);
                stack.Push(focus.Children[2]);
