...
namespace DataStructures
{
    ...
    /// </summary>    
    public sealed class HashedHeap<T> : ISet<T> where T : IComparable<T>
        //TODO:  validate all members of DynamicHeap
    {
        ...
        public HashedHeap(int capacity = DEFAULT_CAPACITY)
        {
            _Table = new List<T>(capacity);
            _Indices = new Dictionary<T, int>(capacity);
        }
        public HashedHeap(IEnumerable<T> items, int capacity = DEFAULT_CAPACITY) : this (Math.Max(capacity, items.Count()))
        {
