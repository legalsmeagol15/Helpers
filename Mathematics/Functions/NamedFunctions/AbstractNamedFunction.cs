...
namespace Mathematics.Functions
{
    ...
    /// </summary>    
    public abstract class AbstractNamedFunction
    {
        /// <summary>
        ...
        {
            AbstractNamedFunction other = obj as AbstractNamedFunction;
        ...
        public static IDictionary<string, AbstractNamedFunction> GetStandardDictionary()
        {
            Dictionary<string, AbstractNamedFunction> returnDict
                = new Dictionary<string, AbstractNamedFunction>();
            foreach (Type t in GetTypesInNamespace(Assembly.GetExecutingAssembly(),
                                                    "Mathematics.Text.NamedFunctions"))
            {
                if (typeof(AbstractNamedFunction).IsAssignableFrom(t))
                {
                    AbstractNamedFunction func = (AbstractNamedFunction)Activator.CreateInstance(t);
